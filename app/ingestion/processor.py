...
from app.services.retrieval.embeddings import  get_embedding_dim, embed_text 
from app.config import settings

logfire.configure(service_name="RAG_Enterprise-ingestion-service")

# Even though the data is stored in the vectorDB but it is also persist stored in the local directory
PROCESSED_DATA_DIR = "processed_data"

# Global Qdrant client instance with proper lifecycle management
_qdrant_client = None


def get_qdrant_client() -> QdrantClient:
    """Get or create the Qdrant client singleton with proper configuration."""
    global _qdrant_client
    if _qdrant_client is None:
        _qdrant_client = QdrantClient(
            url=settings.QDRANT_CLUSTER_ENDPOINT,
            api_key=settings.QDRANT_API_KEY,
            timeout=60.0,  # Increase timeout to prevent connection issues
            prefer_grpc=False,  # Use HTTP/REST instead of gRPC to avoid connection issues
        )
...
def close_qdrant_client():
    """Properly close the Qdrant client connection to avoid RuntimeWarning."""
    global _qdrant_client
    if _qdrant_client is not None:
        try:
            _qdrant_client.close()
            logfire.info("Qdrant client connection closed properly")
        except Exception as e:
            logfire.warning(f"Error closing Qdrant client: {e}")
        finally:
            _qdrant_client = None
...
def run_universal_ingestion(base_dir:str, explicit_source_type:str = None, wipe: bool = False):
    """ 
    Scan the base_dir, map sub-folders to source types, and ingest all documents, 
...
if __name__ == "__main__":
    ...
    try:
        ...
        # Ensure Qdrant client is properly closed to avoid RuntimeWarning
        close_qdrant_client()