...
# Lazy init - Ranker is loaded on first use to ensure the logfire
_ranker = None


def _get_ranker() -> Ranker:
    """
    Init the FlashRank engine lazily. FlashRank use a local OMNX model (ms-macro-MiniLM-L-6-v2) for fast reranking
    """

    global _ranker
    if _ranker is None:
        logfire.info("INTI FlashRank Model locally")
        try:
            # We use spcific cache dir to avoid permission issue
