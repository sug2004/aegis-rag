...
BATCH_SIZE = 50
__GEMINI_DIM = 3072
__FALLBACK_DIM = 768 # all-mpnet-base-v2
...
def _probe_gemini():
    ...
    try:
        ...
        logfire.info("Gemini embeddings ready (gemini-embedding-preview-2, 3072-dim)")
        return model
...
def _load_fallback():
    """ if gemini model fails. then this model if callback """
...
def get_embedding_dim() -> int:
    ...
    _init()
    return __GEMINI_DIM if _model_type == "gemini" else __FALLBACK_DIM
...
def embed_query(query:str) -> list[float]:
    """ Embed the retrival query for semantic similarity search """
    _init()
    if _model_type == "gemini":
...
def embed_text(texts: list[str]) -> list[list[float]]:
    """ Embedd the extracted test from the documents in batches using the above embed_batch function """
    _init()
